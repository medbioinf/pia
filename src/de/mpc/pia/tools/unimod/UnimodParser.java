...
public class UnimodParser {
	...
	private static String packagedUnimod =  "./unimod.xml";
	...
	public UnimodParser() throws JAXBException {
		InputStream stream = null;
		
		try {
			stream = new URL("http://www.unimod.org/xml/unimod.xml").openStream();
		} catch (Exception e) {
			logger.info("Could not get web version of unimod, using local");
			stream = this.getClass().getResourceAsStream(packagedUnimod);
		}
		
		if (stream != null) {
			JAXBContext context = JAXBContext.newInstance(
					UnimodT.class.getPackage().getName());
			Unmarshaller um = context.createUnmarshaller();
	        @SuppressWarnings("unchecked")
			JAXBElement<UnimodT> doc =
	        		(JAXBElement<UnimodT>)um.unmarshal( stream );
			
			modifications = doc.getValue().getModifications().getMod();
		} else {
			logger.error("could not get unimod information!");
		}
	}
